...
<nav class="nav-containner" >
    ...
    <div class="container-home">
        <p class="breadcrumb"><i></i>推荐位管理</p>
        ...
        <div class="table-wrapper">
            ...
            <table cellpadding="0" cellspacing="0" border="0">
                ...
                <?php if(is_array($positions)): $i = 0; $__LIST__ = $positions;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$position): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><span class="btn-status" atr-id="<?php echo ($position["id"]); ?>" atr-message="是否更改状态" atr-status=<?php if(($position["status"]) == "0"): ?>1<?php else: ?>0<?php endif; ?>><?php echo (getStatus($position["status"])); ?></span></td>
                        <td>
                            <a href="javascript:void(0)" class="btn-update" id="btn-Update-<?php echo ($position["id"]); ?>" title="编辑" atr-id="<?php echo ($position["id"]); ?>"><i></i></a>
                            <a href="javascript:void(0)" class="btn-delete" id="btn-Delete-<?php echo ($position["id"]); ?>" title="删除" atr-id="<?php echo ($position["id"]); ?>" atr-message="是否确定删除"><i></i></a>
                        </td>
...
<script type="text/javascript">
    var SCOPE = {
        'add_url' : "/imoocnews/admin/position/add",
        'edit_url' : "/imoocnews/admin/position/edit",
        "set_status_url":"/imoocnews/admin/position/setstatus",
    }
</script>