...
throw方法主要是配合 Generator 函数使用，一般的遍历器对象用不到这个方法。请参阅《Generator函数》一章。
...
// standard: ECMA-262
```
上面代码演示了如何遍历 Set 结构和 Map 结构。值得注意的地方有两个，首先，遍历的顺序是按照各个成员**被添加进数据结构的顺序**。其次，Set 结构遍历时，返回的是一个值，而 Map 结构遍历时，返回的是一个数组，该数组的两个成员分别为当前 Map 成员的键名和键值。
```
...
for (let key of Object.keys(obj)) {  //区别
  console.log(key);
}

var arr = ['foo','bar','baz'];

for (let key of arr.keys()) {        //区别
  console.log(key);
}
```
